...
export const buildElementColorMap = (svgString) => {
  ...
  try {
    ...
    const processElementColors = (element, elementPath, isRootSvg = false) => {
      ...
      if (effectiveFill) {
        ...
        if (normalizedFill && typeof normalizedFill === 'object' && normalizedFill.type === 'GRADIENT_OR_PATTERN') {
          ...
          // Only add to colorMap if not root SVG (root SVG provides inheritance only)
          if (!isRootSvg) {
            if (!colorMap[elementPath]) colorMap[elementPath] = {};
            colorMap[elementPath].fill = {
      ...
      } else if (!isRootSvg && (element.tagName.toLowerCase() === 'path' || element.tagName.toLowerCase() === 'circle' || 
      ...
      if (effectiveStroke) {
        ...
        if (normalizedStroke && typeof normalizedStroke === 'object' && normalizedStroke.type === 'GRADIENT_OR_PATTERN') {
          ...
          if (!isRootSvg) {
            ...
            colorMap[elementPath].stroke = strokeData;
          }
      ...
      } else if (!isRootSvg && (element.tagName.toLowerCase() === 'line' || element.tagName.toLowerCase() === 'polyline')) {
    ...
    // Check if any child elements inherit colors from root SVG
    let rootHasInheritedColors = false;
    ...
    if (rootFill || rootStroke) {
      colorableElements.forEach(el => {
        ...
        const directStroke = el.getAttribute('stroke');
        
        // If child element lacks direct fill/stroke, it will inherit from root
        if ((rootFill && !directFill) || (rootStroke && !directStroke)) {
          rootHasInheritedColors = true;
        }
      });
    }
    
    // Process root SVG element only if its colors are actually inherited by children
    // Mark it as root SVG so it doesn't create customizable color slots
    if (rootHasInheritedColors) {
      processElementColors(svgElement, 'svg', true);
    }
