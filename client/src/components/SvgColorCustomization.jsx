...
// Custom tiled color picker for group adjustments
const TiledGroupColorPicker = ({ displayItems, onColorChange, groupId, onPreviewStart, onPreviewEnd, onPreviewPulse, style }) => {
  const [isPickerOpen, setIsPickerOpen] = useState(false);
  const [hsva, setHsva] = useState({ h: 0, s: 0, v: 0, a: 1 });
  const pickerRef = React.useRef(null);
  
  
  // Get representative color for the picker and convert to HSVA
  const representativeHex = displayItems.length > 0 && displayItems[0].type === 'color' ? displayItems[0].value : '#000000';
  
  // Update HSVA when representative color changes
  useEffect(() => {
    try {
      if (!representativeHex || typeof representativeHex !== 'string') {
        setHsva({ h: 0, s: 0, v: 0, a: 1 });
        return;
      }
      
      let alpha = 1;
      let cleanHex = representativeHex;
      
      // Handle 8-digit hex with alpha
      if (representativeHex.length === 9) {
        cleanHex = representativeHex.substring(0, 7);
        alpha = parseInt(representativeHex.substring(7), 16) / 255;
      }
      
      const newHsva = hexToHsva(cleanHex);
      newHsva.a = alpha;
      setHsva(newHsva);
    } catch (error) {
      setHsva({ h: 0, s: 0, v: 0, a: 1 });
  ...
  const handleColorChange = (colorResult) => {
    ...
    const hex = hsvaToHex(colorResult.hsva);
    let finalHex = hex;
    
    // Add alpha if not fully opaque
    if (colorResult.hsva.a < 1) {
      const alphaHex = Math.round(colorResult.hsva.a * 255).toString(16).padStart(2, '0');
      finalHex = hex + alphaHex;
    }
    ...
    <div ref={pickerRef} style={{ position: 'relative', display: 'inline-block' }}>
      <div
        ...
        onClick={() => {
          ...
          if (onPreviewPulse) onPreviewPulse(groupId);
        }}
        onMouseEnter={() => {
          if (onPreviewStart) onPreviewStart(groupId, 'hover');
        }}
        onMouseLeave={() => {
      ...
      >
        {displayItems.map((item, index) => {
          if (item.type === 'gradient') {
            // For gradients, we need special handling to ensure they show properly in narrow tiles
            return (
              <div 
                key={index}
                style={{ 
                  ...
                  minWidth: '16px', // Slightly wider minimum for gradients
...
// Color picker wrapper using @uiw/react-color Chrome component
const ColorPickerWrapper = ({ value, onChange, disabled, style, onMouseEnter, onMouseLeave, onClick }) => {
  const [isOpen, setIsOpen] = useState(false);
  const pickerRef = React.useRef(null);
  
  // Convert hex to HSVA format for the Chrome picker
  const parseHexToHsva = (hex) => {
    ...
    if (hex.length === 9) {
      ...
      alpha = parseInt(hex.substring(7), 16) / 255;
    }
    
    // Use the imported hexToHsva function
    const hsva = hexToHsva(cleanHex);
    hsva.a = alpha;
    return hsva;
  };
  
  const [hsva, setHsva] = useState(() => parseHexToHsva(value));
  
  // Update when value prop changes
  useEffect(() => {
    setHsva(parseHexToHsva(value));
  }, [value]);
  ...
  const handleColorChange = (colorResult) => {
    ...
    if (onChange) {
      ...
      onChange({ target: { value: finalHex } });
    }
  };
...
const SvgColorCustomization = ({ 
  ...
  const [openGroupPickers, setOpenGroupPickers] = useState({}); // Track which group pickers are open
  ...
  useEffect(() => {
    console.log('[GRADIENT UI DEBUG] SvgColorCustomization received gradientDefinitions:', gradientDefinitions);
    console.log('[GRADIENT UI DEBUG] SvgColorCustomization received elementColorMap:', elementColorMap);
  ...
  // Helper function to get all affected attributes for a given identifier
  const getAffectedAttributes = (elementIdentifier) => {
    const attributes = [];
    
    if (elementIdentifier.startsWith('group-')) {
      // It's a color group - find all attributes with this color
      const originalColor = elementIdentifier.replace('group-', '');
      
      // Check solid color groups
      console.log(`[DEBUG] Looking for originalColor: "${originalColor}"`);
      console.log('[DEBUG] Available solidColorGroups:', Object.keys(solidColorGroups));
      
      // First try to find in solidColorGroups array directly
      const solidSlots = solidColorGroups[originalColor] || [];
      console.log(`[DEBUG] Found ${solidSlots.length} slots in solidColorGroups['${originalColor}']`);
      solidSlots.forEach(slot => {
        console.log(`[DEBUG] Adding attribute: ${slot.elementPath}.${slot.colorType}`);
        attributes.push(`${slot.elementPath}.${slot.colorType}`);
      });
      
      // Also check elementColorMap for direct matches (for backwards compatibility)
      ...
      // We return the attributes that use this gradient so they stay visible
      // while all other attributes get dimmed
    ...
    } else if (elementIdentifier.includes('-')) {
      // It's a specific attribute (e.g., 'path[0]-fill')
      const parts = elementIdentifier.split('-');
      if (parts.length === 2) {
        const [elementPath, colorType] = parts;
        attributes.push(`${elementPath}.${colorType}`);
      }
    } else {
      // It's a single element path - include both fill and stroke if they exist
      if (elementColorMap[elementIdentifier]) {
        if (elementColorMap[elementIdentifier].fill) {
          attributes.push(`${elementIdentifier}.fill`);
        }
        if (elementColorMap[elementIdentifier].stroke) {
          attributes.push(`${elementIdentifier}.stroke`);
        }
      }
    }
  ...
  // Helper function to determine if a group contains stroke, fill, or mixed colors
  const getGroupColorType = (groupId) => {
    ...
    elementsInGroup.forEach(elementId => {
      if (elementId.startsWith('group-')) {
        ...
        const solidSlots = solidColorGroups[originalColor] || [];
        solidSlots.forEach(slot => {
          if (slot.colorType === 'stroke') hasStroke = true;
          if (slot.colorType === 'fill') hasFill = true;
        });
        
        // Check gradient groups
        const gradientGroup = gradientGroups[originalColor];
        if (gradientGroup && gradientGroup.elements) {
          gradientGroup.elements.forEach(element => {
            if (element.colorType === 'stroke') hasStroke = true;
            if (element.colorType === 'fill') hasFill = true;
          });
        }
      } else if (elementId.includes('-stroke')) {
        hasStroke = true;
      } else if (elementId.includes('-fill')) {
        hasFill = true;
      }
    });
  ...
  // Helper function to get all affected attributes for a group (e.g., ['path[0].fill', 'circle[1].stroke'])
  const getGroupAffectedAttributes = (groupId) => {
    const attributes = [];
    ...
    elementsInGroup.forEach(elementId => {
      if (elementId.startsWith('group-')) {
        ...
        if (foundSlot) {
          attributes.push(`${foundSlot.elementPath}.${foundSlot.colorType}`);
        }
      } else if (elementId.includes('-')) {
        // Direct slot identifier (e.g., 'path[0]-fill', 'circle[1]-stroke')
        ...
        // Direct element path - include both fill and stroke if they exist
        if (elementColorMap[elementId]) {
          if (elementColorMap[elementId].fill) {
            attributes.push(`${elementId}.fill`);
          }
          if (elementColorMap[elementId].stroke) {
            attributes.push(`${elementId}.stroke`);
          }
        }
      }
    });
  ...
  const getGroupDisplayItems = (groupId) => {
    const elementsInGroup = Object.keys(elementGroups).filter(elementId => 
      elementGroups[elementId] === groupId
    );
    
    if (elementsInGroup.length === 0) {
      return [{ type: 'gradient', value: 'linear-gradient(to right, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #9400d3)' }];
    }
    ...
    elementsInGroup.forEach(elementId => {
      if (elementId.startsWith('group-')) {
        ...
        // Check if this is a gradient group
        const gradientGroup = gradientGroups[originalColor];
        if (gradientGroup && gradientGroup.gradientId) {
          const gradientDef = gradientDefinitions[gradientGroup.gradientId];
          if (gradientDef) {
            // Build CSS gradient string with proper type
            const stops = gradientDef.stops.map(stop => {
              const { hex } = parseColorString(stop.color);
              // Ensure offset has % symbol
              let offset = stop.offset;
              if (!offset.includes('%')) {
                // Convert decimal to percentage if needed
                const numericOffset = parseFloat(offset);
                offset = `${(numericOffset * 100).toFixed(1)}%`;
              }
              return `${hex} ${offset}`;
            }).join(', ');
            
            // Respect the original gradient type
            const gradientCSS = gradientDef.type === 'lineargradient' 
              ? `linear-gradient(90deg, ${stops})`
              : gradientDef.type === 'radialgradient'
              ? `radial-gradient(circle, ${stops})`
              : `linear-gradient(90deg, ${stops})`; // fallback
            
            gradients.add(gradientCSS);
          }
        } else {
          // Handle solid color groups - get current colors instead of original
          const solidSlots = solidColorGroups[originalColor] || [];
          if (solidSlots.length > 0) {
            // Get current colors from the elementColorMap
            solidSlots.forEach(slot => {
              const element = elementColorMap[slot.elementPath];
              if (element && element[slot.colorType]) {
                const { hex } = parseColorString(element[slot.colorType].current);
                colors.add(hex);
              }
            });
          } else {
            // Fallback to original color if no slots found
            colors.add(originalColor.substring(0, 7));
          }
        }
      } else if (elementId.startsWith('stop-')) {
        // Handle individual gradient stops - show just the stop color
        const stopId = elementId.substring(5); // Remove "stop-" prefix
        const lastStopIndex = stopId.lastIndexOf('-stop-');
        
        if (lastStopIndex !== -1) {
          const gradientId = stopId.substring(0, lastStopIndex);
          const stopIndex = stopId.substring(lastStopIndex + 6); // Skip "-stop-"
        ...
        // Handle individual slot colors - get current color from elementColorMap
        let foundSlot = null;
        ...
        if (foundSlot) {
          if (foundSlot.isGradientStop) {
            // For gradient stop slots, get the current color from gradient definitions
            const gradientDef = gradientDefinitions[foundSlot.gradientId];
            if (gradientDef && gradientDef.stops[foundSlot.stopIndex]) {
              const stopColor = gradientDef.stops[foundSlot.stopIndex].color;
              const { hex } = parseColorString(stopColor);
              colors.add(hex);
            }
          } else {
            // For solid color slots, get the current color from elementColorMap
            const element = elementColorMap[foundSlot.elementPath];
            if (element && element[foundSlot.colorType]) {
              const { hex } = parseColorString(element[foundSlot.colorType].current);
              colors.add(hex);
            }
          }
        }
    ...
    // Build items array - colors first, then gradients for consistent left-to-right order
    const items = [];
    uniqueColors.forEach(color => {
      items.push({ type: 'color', value: color });
    });
    uniqueGradients.forEach(gradient => {
      items.push({ type: 'gradient', value: gradient });
    });
    
    return items.length > 0 ? items : [{ type: 'gradient', value: 'linear-gradient(to right, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #9400d3)' }];
  };
  ...
  const applyColorToGroup = (groupId, newColor) => {
    ...
    elementsInGroup.forEach(elementId => {
      ...
      if (elementId.startsWith('group-')) {
        ...
        // console.log('[GROUP COLOR] Handling slot-level color');
        // Handle individual slot colors - find the slot by elementId
        let foundSlot = null;
        ...
        if (foundSlot) {
          // console.log('[GROUP COLOR] Found slot:', foundSlot);
          if (foundSlot.isGradientStop) {
  ...
  const resetGroup = (groupId) => {
    ...
    // Check if this is a gradient group
    const gradientGroup = gradientGroups[groupId];
    if (gradientGroup) {
      // Reset gradient directly
      const updatedGradientDefinitions = { ...gradientDefinitions };
      const originalGradient = originalGradientDefinitions[gradientGroup.gradientId];
      
      if (originalGradient && updatedGradientDefinitions[gradientGroup.gradientId]) {
        updatedGradientDefinitions[gradientGroup.gradientId] = {
          ...updatedGradientDefinitions[gradientGroup.gradientId],
  ...
  if (!providedColorSlots) {
    ...
    if (colors.fill) {
      colorSlots.push({
        ...
        gradientId: colors.fill.gradientId, // Add missing gradientId!
    ...
    if (colors.stroke) {
      colorSlots.push({
        ...
        gradientId: colors.stroke.gradientId, // Add missing gradientId!
  ...
  const handlePreviewStart = (elementIdentifier, mode = 'hover') => {
    ...
    if (onPreviewStateChange) {
      const affectedAttributes = getAffectedAttributes(elementIdentifier);
      console.log('[SLOT-PREVIEW] Element identifier:', elementIdentifier);
      console.log('[SLOT-PREVIEW] Affected attributes:', affectedAttributes);
      ...
      onPreviewStateChange({
        active: true,
        mode: mode,
        affectedAttributes: affectedAttributes, // Changed from affectedPaths
  ...
  const handleGroupPreviewStart = (groupId, mode = 'hover') => {
    ...
    // Get all affected attributes for elements in this group
    const affectedAttributes = getGroupAffectedAttributes(groupId);
    console.log('[GROUP PREVIEW] Affected attributes:', affectedAttributes);
  ...
  const handleGroupColorChange = (originalColor, newHex, newAlpha, isGradient = false) => {
    let parsedAlpha = parseFloat(newAlpha);
  ...
  const renderColorGroup = (originalColor, slots, isGradient = false, gradientGroup = null) => {
    ...
    // For group controls, use first color as representative when they differ
    const groupCurrentColor = allSameColor ? slots[0].currentColor : (slots[0]?.currentColor || groupDisplayColor);
                                ...
                                onClick={() => {
                                  if (slot.isGradientStop) {
                                    ...
                                    if (updatedGradientDefinitions[slot.gradientId] && originalGradient) {
                                      // Reset only this specific stop to its original color
                                      const originalStop = originalGradient.stops[slot.stopIndex];
                                      if (originalStop) {
                                        updatedGradientDefinitions[slot.gradientId] = {
                                          ...updatedGradientDefinitions[slot.gradientId],
                                          stops: updatedGradientDefinitions[slot.gradientId].stops.map((stop, idx) =>
                                            idx === slot.stopIndex ? { ...originalStop } : stop
                                          )
                                        };
                                      }
                                    }
                  ...
                  <div key={slot.id} style={{ marginBottom: '8px' }}>
                    ...
                    <div style={{ 
                      ...
                      <ColorPickerWrapper
                        ...
                        onMouseEnter={() => handlePreviewStart(`${slot.elementPath}-${slot.colorType}`, 'hover')}
                        onMouseLeave={() => handlePreviewEnd()}
                        onClick={() => handlePulseAnimation(`${slot.elementPath}-${slot.colorType}`)}
                        style={{ 
                          width: '28px', 
                          height: '20px'
                        }}
        ...
        {Object.entries(groups).map(([groupId, group]) => {
          ...
          return (
            <div key={groupId} style={{ marginTop: '15px', padding: '10px', background: '#ffffff', borderRadius: '4px', border: '1px solid #dee2e6', position: 'relative' }}>
              ...
              {/* Bottom row with Group Color Picker and Reset button */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                {/* Group Color Picker */}
                {(() => {
                  const displayItems = getGroupDisplayItems(groupId);
                  
                  if (displayItems.length === 1 && displayItems[0].type === 'color') {
                    // Single solid color - use regular color picker
                    return (
                      <ColorPickerWrapper
                    ...
                    // Multiple items or gradients - use custom tiled color picker
                    return (
                      <TiledGroupColorPicker
                        displayItems={displayItems}
                        groupId={groupId}
                        onColorChange={(color) => {
                          applyColorToGroup(groupId, color);
                        }}
                        onPreviewStart={handleGroupPreviewStart}
                        onPreviewEnd={handlePreviewEnd}
                        onPreviewPulse={handleGroupPreviewPulse}
                      />
                    );
                  }
                })()}
