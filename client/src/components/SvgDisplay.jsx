...
const SvgDisplay = ({ 
  ...
  React.useEffect(() => {
    ...
    const processAndSetUrl = async () => {
      try {
        ...
        console.error('[SvgDisplay] Error processing SVG content:', error);
      ...
      gradients.forEach(gradient => {
        ...
        stops.forEach((stop, index) => {
          if (index !== targetStopIndex) {
            ...
            // Preserve the original stop-opacity of the target stop
            // Don't force it to opaque - keep whatever it was originally
            // console.log('[SVG-PREVIEW] Preserving original opacity for stop', index);
          }
        });
      });
  ...
  const applyPreviewEffects = (svgString, preview, elementColorMap) => {
    ...
    // Handle both old (affectedPaths) and new (affectedAttributes) format
    const affectedAttributes = preview.affectedAttributes || [];
    const affectedPaths = preview.affectedPaths || [];
    
    if (!preview.active || (affectedAttributes.length === 0 && affectedPaths.length === 0)) {
      // console.log('[SVG-PREVIEW] No preview to apply - inactive or no affected attributes/paths');
      return svgString;
    }

    // If using old format or no elementColorMap, fall back to old logic
    if (affectedPaths.length > 0 || !elementColorMap) {
      // console.log('[SVG-PREVIEW] Using legacy path-based preview logic');
    ...
    try {
      ...
      // Create a set of affected attributes for quick lookup
      const affectedAttributesSet = new Set(affectedAttributes);
      // console.log('[SVG-PREVIEW] Affected attributes:', Array.from(affectedAttributesSet));

      let modifiedCount = 0;

      // Go through each element in the elementColorMap
      Object.keys(elementColorMap).forEach(elementPath => {
        const element = findElementByPath(svgElement, elementPath);
        if (!element) return;

        const elementData = elementColorMap[elementPath];
        
        // Handle fill attribute
        if (elementData.fill) {
          const fillAttribute = `${elementPath}.fill`;
          const isFillAffected = affectedAttributesSet.has(fillAttribute);
          
          if (preview.mode === 'affected-pulse') {
            // Pulse mode: only modify affected attributes
            if (isFillAffected) {
              const currentOpacity = element.getAttribute('fill-opacity') || '1';
              const newOpacity = parseFloat(currentOpacity) * preview.opacity;
              element.setAttribute('fill-opacity', newOpacity.toString());
              modifiedCount++;
            }
          } else {
            // Hover mode: dim non-affected attributes
            if (!isFillAffected) {
              const currentOpacity = element.getAttribute('fill-opacity') || '1';
              const newOpacity = parseFloat(currentOpacity) * preview.opacity;
              element.setAttribute('fill-opacity', newOpacity.toString());
        ...
        if (elementData.stroke) {
          const strokeAttribute = `${elementPath}.stroke`;
          const isStrokeAffected = affectedAttributesSet.has(strokeAttribute);
          
          if (preview.mode === 'affected-pulse') {
            // Pulse mode: only modify affected attributes
            if (isStrokeAffected) {
              const currentOpacity = element.getAttribute('stroke-opacity') || '1';
              const newOpacity = parseFloat(currentOpacity) * preview.opacity;
              element.setAttribute('stroke-opacity', newOpacity.toString());
              modifiedCount++;
            }
          } else {
            // Hover mode: dim non-affected attributes
        ...
        if (preview.duration && modifiedCount > 0) {
          element.style.transition = `fill-opacity ${preview.duration}ms ease-in-out, stroke-opacity ${preview.duration}ms ease-in-out`;
        }
      });

      // console.log('[SVG-PREVIEW] Modified', modifiedCount, 'attributes');
      return new XMLSerializer().serializeToString(svgElement);
    } catch (error) {
